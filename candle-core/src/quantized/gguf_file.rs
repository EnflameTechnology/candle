...
fn read_compat_qtensor<R: std::io::Seek + std::io::Read>(
    ...
    let tensor = Tensor::from_vec(values, shape.dims(), device)?;
    QTensor::quantize(&tensor, dst_ggml_dtype)
}
...
impl Content {
    pub fn read<R: std::io::Seek + std::io::Read>(reader: &mut R) -> Result<Self> {
        ...
        for _idx in 0..tensor_count {
            ...
            let (ggml_dtype, src_ggml_dtype) = match raw_ggml_dtype {
                GGML_TYPE_IQ2_XXS | GGML_TYPE_IQ3_XXS | GGML_TYPE_IQ4_XS => {
                    (compat_target_dtype()?, Some(raw_ggml_dtype))
                }
                _ => (GgmlDType::from_u32(raw_ggml_dtype)?, None),
            };
